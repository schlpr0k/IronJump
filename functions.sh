...
## Logging
log_action() {
    local log_file="/var/log/ironjump.log"
    local log_message="$1"
    local timestamp

    timestamp=$(date +"%Y-%m-%d %H:%M:%S")
    echo -e "$timestamp [IronJump]: $log_message" >> "$log_file"
}
...
prompt_reboot() {
    ...
    case "$prompt_response" in
        C|c) echo -e "\n  --> Rebooting server NOW" ; shutdown -r now ; log_action "Reboot initiated through IronJump menu."; exit 0 ;;
...
get_role_type() {
    if [[ -f $IRONJUMP_ROLE_FILE ]]; then
        current_role=$(cat $IRONJUMP_ROLE_FILE |awk '{print $NF}')
    else
...
change_role_type() {
    ...
    if [[ -f $IRONJUMP_ROLE_FILE ]]; then
        ...
        echo -e "\n  -->This host has not been configured with a role.\n  --> Please configure this host as a Server or Endpoint first.\n"
        nav_breaker_bar
    ...
    clear
    echo "IronJump SSH Management - Change Role Wizard"
    nav_top_bar
    echo -e "Current Role: $current_role"
    echo -e "\nSelect a new role for this host: "
    echo "  1. SERVER"
    echo "  2. ENDPOINT"
    echo "  3. HYBRID"
    nav_breaker_bar
    ...
    case $choice in
        1|2|3)
            if [[ $choice == "1" ]]; then
                new_role_choice="SERVER"
            elif [[ $choice == "2" ]]; then
                new_role_choice="ENDPOINT"
            else
                new_role_choice="HYBRID"
            fi
            echo -e "## Role: $new_role_choice" > $IRONJUMP_ROLE_FILE
            echo -e "\n  --> Role Changed to: $new_role_choice"
            log_action "Role changed to: $new_role_choice"
            nav_breaker_bar
        ...
        M|m) main_menu ;;
        Q|q) exit 0 ;;
        *) invalid_choice ; change_role_type ;;
    esac
}
...
## Account Enumeration
list_accounts(){
    for account in $(getent passwd |grep "/home/ironjump/$1"|awk -F: '{print $1}' |sort); do
...
f_sshd_restart(){
    ...
    case "$PKG_MANAGER" in
        "apt"|"dnf"|"yum"|"pacman")
            ...
            ;;
        "pkg") service sshd start ; log_action "Restarting SSH Service" ;;
        "pkg_add") rcctl start sshd ; log_action "Restarting SSH Service" ;;
...
f_ssh_hard_os_selection() {
    ...
    echo "5. Ubuntu 18.04"
    echo "6. Ubuntu 16.04"
    echo "7. Debian Bookworm (Debian 12), Raspberry Pi OS"
    echo "8. Debian Bullseye (Debian 11)"
    echo "9. Debian Buster (Debian 10)"
    echo "10. Rocky Linux 9"
    echo "11. RHEL 8 Server / CentOS 8 Server"
    echo "12. RHEL 7 Server / CentOS 7 Server"
    nav_breaker_bar
    read -p "Enter your choice: " choice
    case $choice in
        ...
        11) f_ssh_hard_regen_hostkeys_2 ; f_ssh_hard_s11 ;;
        12) f_ssh_hard_regen_hostkeys_3 ; f_ssh_hard_s12 ;;
...
f_ssh_hard_check() {
    ...
    if [[ -f $SSH_CONF_SYSTEM ]]; then
        ...
        read -p "Enter your choice: " choice
        case "$choice" in
            Yes|yes|Y|y) echo -e "\n\n\t--> Making backup..." ; cp $SSH_CONF_SYSTEM $SSH_CONF_SYSTEM_BACKUP ; log_action "Made backup of sshd_config file during hardening." ;;
            No|no|N|n) echo -e "\n\n\t--> Skipping backup..." ; log_action "Skipped backup of sshd_config file during hardening." ;;
    ...
    if [[ -f $SSH_CONF_IRONJUMP ]]; then
        ...
        read -p "Enter your choice: " choice
        case "$choice" in
            Yes|yes|Y|y) echo -e "\r\n\t--> Making backup..." ; cp $SSH_CONF_IRONJUMP $SSH_CONF_IRONJUMP_BACKUP ; log_action "Made backup of zz-ironjump.conf file during setup." ;;
            No|no|N|n) echo -e "\r\n\t--> Skipping backup..." ; log_action "Skipped backup of zz-ironjump.conf file during setup." ;;
            *) invalid_choice ; sleep 1 ; f_ssh_hard_check ;;
        esac
    fi
}
        ...
        Yes|yes|Y|y)
            {
                ...
                log_action "Regenerated ssh keys."
            }
            ;;
        No|no|N|n) return 0 ; log_action "Skipped regeneration of keys." ;;
...
f_ssh_hard_regen_hostkeys_3() {
    ...
    f_ssh_hard_regen_hostkeys_2
    log_action "SSH hardening complete. Saved to /etc/ssh/sshd_config.d/zz-ironjump-crypt.conf."
}
...
f_ssh_set_configuration() {
    ...
    while true; do
        ...
        case $choice in
            YES|Yes|yes|Y|y)
                ...
                log_action "Password authentication set to: YES"
                break
                ;;
            NO|No|no|N|n)
                echo -e "\n    --> PasswordAuthentication set to \"NO\"\n" ; default_pass_auth="no"
                log_action "Password authentication set to: NO"
                break
                ;;
            *)
    ...
    echo -e "AcceptEnv LANG LC_* ac_host ac_key" >>$SSH_CONF_IRONJUMP
    log_action "Initial configuration of $SSH_CONF_IRONJUMP configuration complete."
}
...
#Normalize package names per platform
normalize_packages() {
    local pkg="$1"
    case "$pkg" in
        "openssh-client")
            case "$PKG_MANAGER" in
                dnf|yum|rpm) echo "openssh-clients" ;;
                pacman) echo "openssh" ;;
                *) echo "$pkg" ;;
            esac ;;
        "openssh-server")
        ...
        "ssh")
            case "$PKG_MANAGER" in
                pacman|dnf|yum|rpm) echo "openssh" ;;
                *) echo "$pkg" ;;
            esac ;;
        "diceware")
            case "$PKG_MANAGER" in
                pacman) echo "python-diceware" ;;  # Correct for Arch-based systems
                *) echo "$pkg" ;;
            esac ;;
        *)
            echo "$pkg"
...
f_setup_ironjump() {
    local roletype
    local halt_deployment
    local REQUIRED_PACKAGES
    local all_installed

    #Establish logging for IronJump Server
    touch /var/log/ironjump.log
    chown root:root /var/log/ironjump.log
    chmod 0600 /var/log/ironjump.log

    #Check for internet connectivity
    log_action "Begin IronJump server deployment and setup."
    f_check_internet_connectivity

    #Install required packages
    REQUIRED_PACKAGES=("gcc" "make" "lsof" "ssh" "openssh-client" "openssh-server" "diceware")
    all_installed=true

    for pkg in "${REQUIRED_PACKAGES[@]}"; do
        norm_pkg=$(normalize_packages "$pkg")

        case "$PKG_MANAGER" in
            apt)
                dpkg-query -W -f='${Status}' "$norm_pkg" 2>/dev/null | grep -q "install ok installed" || all_installed=false
                ;;
            dnf|yum)
                rpm -q "$norm_pkg" >/dev/null 2>&1 || all_installed=false
                ;;
            pacman)
                pacman -Q "$norm_pkg" >/dev/null 2>&1 || all_installed=false
                ;;
            pkg)
                pkg info "$norm_pkg" >/dev/null 2>&1 || all_installed=false
                ;;
            pkg_add)
                pkg_info "$norm_pkg" >/dev/null 2>&1 || all_installed=false
                ;;
        esac
    done

    if [[ "$check_internet" == "0" ]] && [[ "$all_installed" == "false" ]]; then
        echo -e "Packages Required: ${REQUIRED_PACKAGES[@]}"
        read -p "Halting deployment due to missing packages. Press [ENTER] to return to Main Menu." halt_deployment
        main_menu
    fi

    if [[ "$check_internet" == "1" ]]; then
        f_sshd_setup
        log_action "Installed SSH server and client."
        f_diceware_setup
        log_action "Installed Diceware utility."
        f_lsof_setup
        log_action "Installed lsof utility."
        f_make_setup
        log_action "Installed make utility."
    fi

    harden_ssh_service
    log_action "Hardened SSH services."
    ...
    log_action "Created IronJump home directories."
    ...
    unset sync_pass
    log_action "Creation of ironsync account complete."
    ...
    log_action "Creation of IronJump groups complete."
...
EOF
    ...
    log_action "  --> Configuration of $SSH_CONF_IRONJUMP file complete."
...
if [[ -z \$ac_host ]]; then
    ...
    log_action "Sync Error: Host attempted to sync, but no hostname provided."
    exit 1
elif [[ ! -f "\$ac_host_file" ]]; then
    echo "Error: File does not exist or permission denied."
    log_action "Sync Error: \$ac_host failed sync. No access control file found."
    exit 1
fi

if [[ -z \$ac_key ]]; then
    echo "Error: No access control key provided."
    log_action "Sync Error: \$ac_host failed sync. No key submitted during sync."
    exit 1
elif [[ ! \$(cat /home/ironjump/endpoints/chroot/\$ac_host/.ssh/ac_key) == \$ac_key ]]; then
    echo "Error: Access control key does not match host."
    log_action "Sync Error: \$ac_host failed sync. Key mismatch."
    exit 1
fi
...
EOF
    ...
    log_action "Setup of IronJump synchronization service complete."

    #Assign role
    if [[ -f $IRONJUMP_ROLE_FILE ]]; then roletype=$(cat $IRONJUMP_ROLE_FILE |awk '{print $NF}'); else roletype="UNASSIGNED"; fi
    if [[ ! $roletype == "HYBRID" ]]; then echo -e "## Role:\tSERVER" > $IRONJUMP_ROLE_FILE; log_action "Role set to: SERVER"; fi
    chmod 600 $IRONJUMP_ROLE_FILE
    log_action "IronJump Server Deployment Complete."
    nav_breaker_bar
    read -p "Setup Complete. Press [ENTER] to continue."
}

## Generate an archive of files and logs
ironjump_archive() {
    local continue
    local encrypt_choice
    local archive_del_choice
    local archive_dir="/opt/IronJump/ironjump_archive"
    if [[ ! -d $archive_dir ]]; then mkdir $archive_dir; fi
    local archive_name="$(date '+%Y-%m-%d_%H-%M')_ironjump_archive.tar.gz"
    local archive_name_encrypted="$(date '+%Y-%m-%d_%H-%M')_ironjump_archive.tar.gz.enc"
    local folders=(
        "/root"
        "/home"
        "/opt/IronJump"
    ...
    clear
    echo "IronJump SSH Management - Files and Logs Archive Wizard"
    nav_top_bar
    #Check for existing archive
    if [[ -f $archive_dir/$archive_name ]] || [[ -f $archive_dir/$archive_name_encrypted ]]; then
        nav_break_bar
        echo -e "\nArchive already exists."
        read -p "  --> Do you want to delete the previous archive? (yes/no): " archive_del_choice
        archive_del_choice=$(echo $archive_del_choice |tr [[:upper:]] [[:lower:]])
        case $archive_del_choice in
            y|yes) rm -f $archive_dir/{$archive_name,$archive_name_encrypted} ; log_action "Deleted previous archive." ; echo -e "    --> Previous archive deleted." ;;
            n|no) nav_breaker_bar ; read -p "  --> Aborting process. Press [ENTER] to return to Main Menu" continue ; main_menu ;;
            *) invalid_choice ; ironjump_archive ;;
        esac
    fi
    ...
    echo -e "\n  --> Generating archive."
    tar --recursive --same-owner -czf $archive_dir/$archive_name ${folders[@]}
    if [[ $? -ne 0 ]]; then
        log_action "Attempt to archive files and logs failed."
        echo -e "  --> Something went wrong creating archive. Aborting process."
        nav_breaker_bar
        read -p "  --> Press [ENTER] to return to Main Menu" continue
        main_menu
    else
        log_action "System archived to $archive_dir/$archive_name"
        echo -e "  --> Archive complete. Saved to: $archive_dir/$archive_name\n"
    fi

    #Prompt for encryption
    read -p "Do you want to encrypt the archive? (yes/no): " encrypt_choice
    encrypt_choice=$(echo $encrypt_choice |tr [[:upper:]] [[:lower:]])
    case $encrypt_choice in
        y|yes)
            echo -e "  --> Encrypting archive."
            openssl enc -aes-256-cbc -e -pbkdf2 -in "$archive_dir/$archive_name" -out "$archive_dir/$archive_name_encrypted"
            if [[ $? -ne 0 ]]; then
                log_action "Attempt to encrypt archive failed. Unencrypted archive created."
                echo -e "  --> Something went wrong encrypting archive. Unable to complete process."
                echo -e "  --> Unencrypted archive generated: $archive_dir/$archive_name"
                nav_breaker_bar
                read -p "Press [ENTER] to return to Main Menu." continue
                main_menu
            else
                log_action "Encrypted archive generated: $archive_dir/$archive_name_encrypted."
                rm -f $archive_dir/$archive_name
                chmod 0400 $archive_dir/$archive_name_encrypted
                echo -e "  --> Encryption complete. Saved to: $archive_dir/$archive_name_encrypted"
            fi
            ;;
        n|no) echo -e "  --> File will not be encrypted.\n  --> Unencrypted archive generated: $archive_dir/$archive_name." ; $nav_breaker_bar ; read -p "Press [ENTER] to return to Main Menu." continue ; main_menu ;;
        *) invalid choice ; echo -e "  --> File will not be encrypted.\n  --> Unencrypted archive generated: $archive_dir/$archive_name." ; $nav_breaker_bar ; read -p "Press [ENTER] to return to Main Menu." continue ; main_menu ;;
    esac

    #Provide instructions for decrypting
    nav_breaker_bar
    echo -e "  To decrypt the archive, run the following commands:"
    echo -e "    --> openssl enc -aes-256-cbc -d -pbkdf2 -in $archive_dir/$archive_name_encrypted -out $archive_name"
...
f_sshd_setup() {
    ...
    case "$PKG_MANAGER" in
        "apt") apt install -y openssh-client openssh-server ssh ;;
...
f_lsof_setup(){
    ...
    case "$PKG_MANAGER" in
        "apt") apt install -y lsof ;;
        "yum") yum install -y lsof ;;
        "dnf") dnf install -y lsof ;;
        "pacman") pacman -Syu --noconfirm lsof ;;
        "pkg_add") pkg_add lsof ;;
        *) echo "  --> Package manager not recognized. Cannot install lsof."; return 1 ;;
    esac
}

f_build_systemd_services(){
    cat <<EOF > "/etc/systemd/system/ironjump.service"
[Unit]
Description=IronJump Autossh Reverse Tunnel
After=network-online.target
Wants=network-online.target

[Service]
Type=simple
ExecStart=/bin/ironjump-connect.sh
Restart=always
RestartSec=15
User=root
Environment=AUTOSSH_GATETIME=0
Environment=AUTOSSH_LOGFILE=/var/log/ironjump-autossh.log

[Install]
WantedBy=multi-user.target
EOF

    cat <<EOF > "/etc/systemd/system/ironjump-sync.service"
[Unit]
Description=IronJump Sync Script

[Service]
Type=oneshot
ExecStart=/bin/ironjump-sync.sh
User=root
EOF

    cat <<EOF > "/etc/systemd/system/ironjump-sync.timer"
[Unit]
Description=Run IronJump Sync Script Every 5 Minutes
...
view_configuration() {
    ...
    if [[ -f $SSH_CONF_IRONJUMP ]]; then
        ...
        log_action "WARNING: $SSH_CONF_IRONJUMP file not found."
        nav_breaker_bar
        read -p "Press [ENTER] to continue." continue
    fi
    deployment_menu
...
mod_configuration() {
    ...
    if [[ -f $SSH_CONF_IRONJUMP ]]; then
        ...
        log_action "Manual configuration of IronJump SSHD configuration file detected."
    else
        echo "  --> Configuration file not found."
        log_action "WARNING: $SSH_CONF_IRONJUMP file not found."
        nav_breaker_bar
...
usr_mgmt_new() {
    ...
    if [[ ! "$usr_new_set_pubkey" =~ ^ssh-ed25519\  ]] && [[ ! "$usr_new_set_pubkey" =~ ^ssh-rsa\  ]]; then
        echo -e "    --> SSH Pubkey must be ed25519 (recommended) or RSA. Aborting user creation."
        nav_breaker_bar
        log_action "Failed to create user account: $usr_new_set_name. Bad SSH key type."
    ...
    case "$choice" in
        A|a) log_action "User creation of $usr_new_set_name aborted by admin." ; root_server_user_acct_mgmt_menu ;;
        C|c)
            ...
            log_action "New user account $usr_new_set_name created."
            ;;
...
local_account_check(){
    local continue
    local account_check
    # Function requires 3 inputs when called 1) {users or endpoints} and 2) {username} and 3) menu location if error
...
usr_mgmt_enable() {
    ...
    if [[ $(echo $choice | tr [[:upper:]] [[:lower:]]) == "confirm" ]]; then
        ...
        log_action "User account $usr_set_enable_name is now enabled."
        echo -e "\n    --> User $usr_set_enable_name is now enabled!"
        nav_breaker_bar
        read -p "Press [ENTER] to continue." continue
        root_server_user_acct_mgmt_menu
    else
        log_action "Attempt to enable user account $usr_set_enable_name failed. No confirmation given."
...
usr_mgmt_disable() {
    ...
    if [[ $(echo $choice | tr [[:upper:]] [[:lower:]]) == "confirm" ]]; then
        ...
        sed -i "/$usr_set_disable_name:/s/:Allowed:.*$/:Disabled:U:$(date +%Y-%m-%d)/" /home/ironjump/endpoints/chroot/*/access_control
        log_action "User account $usr_set_disable_name is now disabled."
        echo -e "\n    --> User $usr_set_enable_name is now disabled!"
        nav_breaker_bar
        read -p "Press [ENTER] to continue." continue
        root_server_user_acct_mgmt_menu
    else
        log_action "Attempt to disable user account $usr_set_disable_name failed. No confirmation given."
        read -p "    --> Confirmation not given. Press [ENTER] to return to the User Account Management Menu." continue
        root_server_user_acct_mgmt_menu
    fi
}
...
usr_mgmt_delete() {
    ...
    if [[ $(echo $choice | tr [[:upper:]] [[:lower:]]) == "confirm" ]]; then
        ...
        sed -i "/$usr_set_delete_name:/s/:Allowed:.*$/:Deleted:U:$(date +%Y-%m-%d)/" /home/ironjump/endpoints/chroot/*/access_control
        log_action "User account $usr_set_enable_name has been deleted."
        echo "    --> User $usr_set_delete_name has been deleted!"
        nav_breaker_bar
        read -p "Press [ENTER] to continue." continue
        root_server_user_acct_mgmt_menu
    else
        log_action "Attempt to delete user account $usr_set_delete_name failed. No confirmation given."
        nav_breaker_bar
...
usr_mgmt_mod_pubkey() {
    ...
    if [[ ! "$usr_set_update_pubkey_key" =~ ^ssh-ed25519\  ]] && [[ ! "$usr_set_update_pubkey_key" =~ ^ssh-rsa\  ]]; then
        ...
        log_action "Failed to update user account $usr_set_update_pubkey_name's public key. Invalid key type detected."
        nav_breaker_bar
        read -p "Press [ENTER] to continue. Returning to User Account Menu" continue
        root_server_user_acct_mgmt_menu
    else
    ...
    if [[ $(echo $choice | tr [[:upper:]] [[:lower:]]) == "confirm" ]]; then
        ...
        log_action "Successfully updated user account $usr_set_update_pubkey_name's public key."
        echo -e "\n    --> User $usr_set_enable_name's public key has been modified!"
        nav_breaker_bar
        read -p "Press [ENTER] to continue. Returning to User Account Menu" continue
        root_server_user_acct_mgmt_menu
    else
        log_action "Failed to update user account $usr_set_update_pubkey_name's public key. No confirmation given."
        nav_breaker_bar
        read -p "  --> Confirmation not given. Press [ENTER] to return to the User Account Menu." continue
        root_server_user_acct_mgmt_menu
    fi
...
usr_mgmt_set_expire() {
    ...
    if [[ "$usr_set_expire_date" == "never" ]]; then
        ...
        log_action "User account $usr_set_expire_name set to never expire."
        ...
        log_action "User account $usr_set_expire_name set to expire on $usr_set_expire_date."
...
dev_mgmt_new() {
    ...
    #Create random ingot number
    while :; do
        local ep_base="ingot-"
        local ep_num=$((50000 + RANDOM % 10000))
        local ep_account_name="$ep_base$ep_num"
        if [[ -z $(id $ep_account_name 2>/dev/null) ]]; then
            break
        fi
    done
    ...
    log_action "Endpoint account $ep_account_name created."
...
EOF
    ...
    #Create ac_key for synchronization validation
    local ac_key_new=$(cat /dev/urandom | tr -dc 'A-Z2-79' | fold -w4 | head -n3 | paste -sd'-' -)
    echo -e "$ac_key_new" >$dev_new_set_sshdir/ac_key
    echo -e "ac_key="\"$ac_key_new\" >>$dev_new_set_homedir/ingot-keys

    #Set Permissions
    chown -R $ep_account_name:$ep_account_name $dev_new_set_homedir
    chmod 750 "$dev_new_set_sshdir"
    ...
    chmod 0640 $dev_new_set_homedir/access_control
    chown root:ironsync $dev_new_set_sshdir/ac_key
    chmod 0440 $dev_new_set_sshdir/ac_key
    log_action "Endpoint account $ep_account_name prepared for registration."

    #Display Username and Password Instructions
    clear
    echo "IronJump - New Endpoint Account (Credentials)"
    nav_top_bar
    echo -e "  --> Endpoint account name: "$ep_account_name
    echo -e "  --> One-time Password: "$token
...
dev_mgmt_enable() {
    ...
    nav_breaker_bar
    read -p "  --> Enter an endpoint account to enable: " dev_set_enable_name
    ...
    if [[ $(echo $choice | tr [[:upper:]] [[:lower:]]) == "confirm" ]]; then
        ...
        log_action "Endpoint account $dev_set_enable_name is now enabled."
        echo -e "\n  --> User $dev_set_enable_name is now enabled!"
        nav_breaker_bar
        read -p "Press [ENTER] to continue." continue
        root_server_endpoint_acct_mgmt_menu
    else
        log_action "Attempt to enable endpoint account $dev_set_enable_name failed. No confirmation given."
...
dev_mgmt_disable() {
    ...
    if [[ $(echo $choice | tr [[:upper:]] [[:lower:]]) == "confirm" ]]; then
        ...
        log_action "Endpoint account $dev_set_disable_name is now disabled."
        echo -e "\n    --> User $dev_set_enable_name is now disabled!"
        nav_breaker_bar
        read -p "Press [ENTER] to continue." continue
        root_server_endpoint_acct_mgmt_menu
    else
        log_action "Attempt to disable endpoint account $dev_set_disable_name failed. No confirmation given."
        read -p "    --> Confirmation not given. Press [ENTER] to return to the Endpoint Account Management Menu." continue
        root_server_endpoint_acct_mgmt_menu
    fi
}
...
dev_mgmt_delete() {
    ...
    if [[ $(echo $choice | tr [[:upper:]] [[:lower:]]) == "confirm" ]]; then
        ...
        log_action "Endpoint account $dev_set_delete_name has been deleted."
        echo -e "    --> Endpoint $dev_set_delete_name has been deleted!"
        nav_breaker_bar
        read -p "Press [ENTER] to continue." continue
        root_server_endpoint_acct_mgmt_menu
    else
        log_action "Attempt to delete endpoint account $dev_set_delete_name failed. No confirmation given."
        nav_breaker_bar
...
dev_mgmt_mod_pubkey() {
    ...
    if [[ ! "$dev_set_update_pubkey_key" =~ ^ssh-ed25519\  ]] && [[ ! "$dev_set_update_pubkey_key" =~ ^ssh-rsa\  ]]; then
        echo "    --> SSH Pubkeys must be ed25519 (recommended) or RSA. Aborting modification."
        log_action "Attempt to modify endpoint account $dev_set_update_pubkey_name's pubkey failed. Wrong key type."
        nav_breaker_bar
    ...
    if [[ $(echo $choice | tr [[:upper:]] [[:lower:]]) == "confirm" ]]; then
        ...
        log_action "Endpoint account $dev_set_update_pubkey_name successfully updated."
        nav_breaker_bar
        read -p "Press [ENTER] to continue. Returning to Endpoint Account Menu" continue
        root_server_endpoint_acct_mgmt_menu
    else
        log_action "Attempt to update endpoint account $dev_set_update_pubkey_name's pubkey failed. No confirmation given."
        nav_breaker_bar
        read -p "  --> Confirmation not given. Press [ENTER] to return to the Endpoint Account Menu." continue
        root_server_endpoint_acct_mgmt_menu
    fi
...
dev_mgmt_set_expire() {
    ...
    if [[ "$dev_set_expire_date" == "never" ]]; then
        ...
        log_action "Endpoint account $dev_set_expire_name is set to never expire."
        ...
        log_action "Endpoint account $dev_set_expire_name is set to expire on $dev_set_expire_date."
...
f_check_internet_connectivity() {
    ...
    if ping -c 1 -W 4 1.1.1.1 > /dev/null 2>&1; then
        echo "Success!" ; check_internet="1" ; sleep 1
    else
        echo -e "Denied\n" ; check_internet="0" ; sleep 1
        read -p "Internet connectivity not detected. Continue with install (y/n): " choice
        case $choice in
            y|Y|yes|Yes|YES) echo "Proceeding with setup." ; sleep 1 ;;
            n|n|no|No|NO) echo "Setup halted. Exiting..." ; sleep 1 ; main_menu ;;
...
ep_connect() {
    ...
    #Create log file on endpoint
    if [[ ! -f /var/log/ironjump.log ]]; then
        touch /var/log/ironjump.log
        chown root:root /var/log/ironjump.log
        chmod 0600 /var/log/ironjump.log
    fi
    ...
    f_check_internet_connectivity
    clear
    if [[ ! -f /opt/IronJump/autossh/configure ]]; then read -p "  --> AutoSSH files are not in folder. Cannot deploy endpoint. See README file." continue; main_menu ;fi
    clear
    ...
    read -p "    --> IronJump Server IP address: " ironjump_srv_ip
    read -p "    --> Endpoint account name (Format: ingot-5XXXX): " ironjump_ep_name
    read -p "    --> Were you provided a one-time SSH access password? (yes/no): " has_token
    case $has_token in
        No|no|N|n) read -p "  --> [WARNING] You cannot proceed without a password." ; log_action "Endpoint registration incomplete. No one-time password" ; ep_mgmt_menu ;;
        Yes|yes|Y|y) sleep 1 ;;
        *) invalid_choice ; ep_mgmt_menu ;;
    esac

    #Install necessary packages for IronJump endpoint.
    REQUIRED_PACKAGES=("gcc" "make" "lsof" "ssh" "openssh-client" "openssh-server")
    all_installed=true
    ...
    if [[ "$check_internet" == "0" ]] && [[ "$all_installed" == "false" ]]; then
        echo "Packages Required: ${REQUIRED_PACKAGES[@]}"
        read -p "Halting deployment due to missing packages. Press [ENTER] to return to Main Menu." halt_deployment
        main_menu
    fi
    ...
    ssh -o "StrictHostKeyChecking no" -T $IRONJUMP_EP_NAME@$IRONJUMP_SERVER > /root/ingot-keys
    echo " --> Migrating Keys"; sleep 5
    if [[ ! -f /root/ingot-keys ]]; then
        log_action "Endpoint registration incomplete. Ingot keys file missing."
        echo -e "    --> Ingot Key Missing. Aborting Process."
        nav_breaker_bar
        read -p "Press [ENTER] to return to Main Menu." continue
        main_menu
    fi
    ...
    cat /root/ingot-keys |grep ac_key |cut -d '"' -f2 > /root/.ssh/ac_key
...
#!/bin/bash
###(Modified AutoSSH Command: Removed -f from command for systemd.services to run as foreground process.)
###(Independent execution will fail during troubleshooting unless you put back the -f after -M 0)
autossh -M 0 -o "StrictHostKeyChecking no" -N -i /root/.ssh/ironjump_key -R $(cat /etc/hostname |cut -d '-' -f 2):localhost:22 $(cat /etc/hostname)@$IRONJUMP_SERVER
EOF
...
ironjump_sync() {
    local ac_host
    local ac_key
    ac_host=\$(cat /etc/hostname) ac_key=\$(cat /root/.ssh/ac_key) ssh -i /root/.ssh/ironsync_key -T -o "StrictHostKeyChecking no" -o "SendEnv ac_host ac_key" ironsync@$IRONJUMP_SERVER > /root/.ssh/access_control
    sleep 5
    ...
    case "\$status" in
        "Allowed")
            if [[ "\$user_exists" == "yes" ]]; then
                ...
                echo "\$ssh_key" > "/home/\$username/.ssh/authorized_keys"
                chown -R "\$username:\$username" "/home/\$username/.ssh"
                log_action "User account \$username created."
            fi

            #Set/Remove sudoer  Privileges - Do nothing if no change
            if [[ "\$priv" == "U" ]] && [[ ! -z \$(grep \$username \$SUDOERS_FILE) ]]; then
                sed -i "/\$username /d" \$SUDOERS_FILE
                log_action "User account \$username removed from sudoers file."
            fi
            if [[ "\$priv" == "S" ]] && [[ -z \$(grep \$username \$SUDOERS_FILE) ]]; then
                echo -e "\$username ALL=(ALL) NOPASSWD:ALL" >> "\$SUDOERS_FILE"
                log_action "User account \$username added to sudoers file."
            fi

            #Set/Modify Account Expiration - Do nothing if no change
            local current_exp=\$(chage -l \$username |grep "Account expires" |awk '{print \$NF}')

            if [[ "\$expiration" == "DNE" ]]; then
                if [[ ! "\$current_exp"  == "never" ]]; then
                    chage -E -1 "\$username"
                    log_action "User account \$username set to never expire."
                fi
            else
                if [[ ! "\$expiration" == "\$current_exp" ]]; then
                    chage -iE "\$expiration" "\$username"
                    log_action "User account \$username set expiration to \$expiration."
                fi
            fi
            ;;
        "Revoked"|"Disabled")
            if [[ "\$user_exists" == "yes" ]]; then
                ...
                chmod 600 "/home/\$username/.ssh/authorized_keys"
                sed -i "/^\$username /d" "\$SUDOERS_FILE"
                log_action "User account \$username revoked."
            fi
            ;;
        "Deleted")
            if [[ "\$user_exists" == "yes" ]]; then
                ...
                log_action "User account \$username deleted."
            fi
            ;;
    esac
done < "\$ACCESS_FILE"
...
EOF
    ...
    #Build Systemd Services for Autostart
    f_build_systemd_services
    sleep 1; systemctl daemon-reexec
    sleep 1; systemctl daemon-reload
    sleep 1; systemctl enable ironjump
    sleep 1; systemctl enable --now ironjump-sync.timer
    sleep 1; systemctl enable ssh ; systemctl start ; systemctl status ssh

    #Set Endpoint Role
    if [[ -f $IRONJUMP_ROLE_FILE ]]; then roletype=$(cat $IRONJUMP_ROLE_FILE |awk '{print $NF}'); else roletype="UNASSIGNED"; fi
    if [[ ! $roletype == "HYBRID" ]]; then echo -e "## Role:\tENDPOINT" > $IRONJUMP_ROLE_FILE; fi
    log_action "Endpoint account registered with IronJump Server. Rebooting endpoint."
    nav_breaker_bar
    read -p "This endpoint will now reboot. Press [ENTER] to continue." continue
    shutdown -r now
}
...
ep_force_sync(){
    ...
    if [[ ! -f /bin/ironjump-sync.sh ]]; then
        ...
        log_action "Admin forced synchronization of permissions with IronJump server."
    fi
    nav_breaker_bar
...
ep_smelt_device() {
    ...
    echo "--> This is NOT a LOW-LEVEL disk wipe and should not be used for compliance tasks."
    echo -e "\nDOUBLE CHECK AFTER PROCESS COMPLETE. NOT ALL SYSTEMS ARE THE SAME.\nTRUST, BUT VERIFY!\n"
    nav_breaker_bar
    echo "!!! --> DOES NOT WORK FOR VIRTUAL MACHINES w/ SNAPSHOTS ENABLED <-- !!!"
    nav_breaker_bar
    ...
    #Kill all SSH connections to prevent remote intervention
    echo "[*] Killing all SSH connections..."
    pkill -u $(whoami) sshd
    sleep 2

    #Overwrite all disks with random data to prevent recovery
    ...
    #Overwrite swap space (if enabled)
    echo "[*] Wiping swap..."
    swapoff -a
    dd if=/dev/urandom of=/dev/zero bs=1M count=100

    #Destroy all partitions and filesystems
    ...
    #Remove critical system files to render the OS unbootable
    echo "[*] Deleting system files..."
    rm -rf /boot /bin /sbin /lib /lib64 /usr /etc /home /var /root

    #Overwrite system logs and memory to eliminate traces
...
assignments_add_by_user() {
    ...
    log_action "Access Control - User account $assign_user allowed $assign_set_priv access to $assign_endpoint. Expiration set to $assign_set_exp."
        ...
        1)
            ...
            case $choice in
                C|c)
                    ...
                    ;;
                A|a)
                    log_action "Access Control - Failed attempt to revoke user account $assign_user from endpoint $assign_endpoint. No confirmation given."
                    read -p "    --> Aborting revocation. Returning to Access Control Menu." continue
                    access_control_mgmt_menu
                    ;;
        ...
        2)
            ...
            case $choice in
                C|c)
                    ...
                    log_action "Access Control - User account $assign_user revoked on from all endpoints."
                    nav_breaker_bar
                    read -p "    --> Revocation complete. Press [ENTER] to return to Access Control Menu." continue
                    access_control_mgmt_menu
                    ;;
                A|a)
                    echo -e "\n    --> Aborting revocation. Returning to Access Control Menu."
                    log_action "Access Control - Failed attempt to revoke user account $assign_user on all endpoints. No confirmation given."
                    nav_breaker_bar
                    read -p "Press [ENTER] to return to Access Control Menu." continue
                    access_control_mgmt_menu
                    ;;
    ...
    case $choice in
        1)
            ...
            case $choice in
                C|c)
                    sed -i "/$assign_user:/s/:Allowed:.*$/:Revoked:U:$(date +%Y-%m-%d)/" /home/ironjump/endpoints/chroot/$assign_endpoint/access_control
                    log_action "Access Control - User account $assign_user revoked access from endpoint $assign_endpoint."
                    echo -e "\n  --> Revocation complete.\nNote: Changes may take up to 5 minutes."
                    nav_breaker_bar
                    read -p "Press [ENTER] to return to Access Control Menu" continue
                    access_control_mgmt_menu
                    ;;
                A|a)
                    log_action "Access Control - Attempt to revoke user account $assign_user's access on endpoint $assign_endpoint failed. No confirmation given."
                    read -p "  --> Aborting revocation. Returning to Access Control Menu." continue
                    access_control_mgmt_menu
                    ;;
        ...
        2)
            ...
            case $choice in
                C|c)
                    ...
                    log_action "Access Control - User account $assign_user revoked from all endpoints."
                    echo -e "\n  --> Revocation complete.\nNote: Changes may take up to 5 minutes."
                    nav_breaker_bar
                    read -p "Press [ENTER] to return to Access Control Menu" continue
                    access_control_mgmt_menu
                ...
                A|a)
                    echo -e "\n  --> Aborting revocation. Returning to Access Control Menu."
                    log_action "Access Control - Attempt to revoke user account $assign_user from all endpoints failed. No confirmation given."
                    nav_breaker_bar
                    read -p "Press [ENTER] to return to Access Control Menu" continue
                    access_control_mgmt_menu
                    ;;
        ...
        1)
            ...
            case $choice in
                C|c)
                    ...
                    log_action "Access Control - User account $assign_user allowed access to all endpoints with $assign_set_priv access. Account expiration set to: $assign_set_exp."
                ...
                A|a)
                    ...
                    log_action "Access Control - Attempt to allow user account $assign_user to all endpoints failed. Process aborted by admin."
                    nav_breaker_bar
                    read -p "Press [ENTER] to return to Access Control Menu" continue
                    access_control_mgmt_menu
                    ;;
        ...
        2)
            ...
            case $choice in
                ...
                A|a)
                    ...
                    log_action "Access Control - Attempt to revoke user account $assign_user to all endpoints failed. Process aborted by admin."
                    nav_breaker_bar
                    read -p "Press [ENTER] to return to Access Control Menu" continue
                    access_control_mgmt_menu
                    ;;
...
access_control_revoke_all_assignments(){
    local continue
    local choice
    clear
    ...
    if [[ $(echo $choice | tr [[:upper:]] [[:lower:]]) == "confirm" ]]; then
        find /home/ironjump/endpoints/chroot/* -type f -name "access_control" -exec sed -i "s/:Allowed.*/:Revoked:U:$(date +%Y-%m-%d)/g" {} +
        log_action "Access Control - ALL USERS REVOKED FROM ALL ENDPOINTS."
        ...
        log_action "Access Control - Attempt to revoke all users from all endpoints failed. Process aborted by admin."
...
validate_date() {
    ...
    local input_date
    local today_day
    # NOTE: Requires two inputs when called "{user or endpoint or assignment}" and "date in format YYYY-MM-DD" for confirmation and navigation upon error.
    read_input_date=$2
